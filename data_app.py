...
def load_viz(df):
        ...
        st.write(sns.countplot(x='airline_sentiment', data=df,ax=ax))
        ...
        fig, ax = plt.subplots(figsize=(15,5))
        st.write(sns.countplot(x='airline', data=df,ax=ax))
        st.pyplot(fig)
        ...
        fig, ax = plt.subplots(figsize=(15,5))
        st.write(sns.countplot(x='airline_sentiment', order=['neutral', 'positive', 'negative'], data=temp_df,ax=ax))
        st.pyplot(fig)
